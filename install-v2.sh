...
# Send errors to vm-build.log and return succes or error about each process
exe () {
...
---------------------> PREREQUISITES <--------------------
     "
command -v virtualbox >./.install.log 2>&1 || { 
    echo >&2 "I require virtualbox but it's not installed.  So i will do it !"; 
    exe '--- Install Virtualbox ---'     sudo apt install virtualbox -y;
    exit 1; 
    }
echo "
----------------------------------------------------------
     "
command -v vagrant >./.install.log 2>&1 || { 
    echo >&2 "I require vagrant but it's not installed.  So i will do it !"; 
    exe '--- Install Vagrant ---'     sudo apt install vagrant -y;
    exit 1; 
    }
echo "
----------------------------------------------------------
...
echo "Or press 'enter' to select default name : 'data'
     "
read PROJECTFOLDER
# if the first value is empty the second is selected
PROJECTFOLDER=${PROJECTFOLDER:-data}
# regex for remove space from variable
PROJECTFOLDER=${PROJECTFOLDER//[[:blank:]]/}
echo "----------------------------------------------------------"
...
--------------------------> 2/4 <-------------------------
     "
echo "> Choose vm box if you know exact name :"
echo "Or press 'enter' to select default box : 'ubuntu/xenial64'
     "
echo -e "\e[1m !!! If you don't know, just select the default value !!! \e[21m
     "
read VMBOX
# if the first value is empty the second is selected
...
echo "Or press 'enter' to select default value : '83'
     "
read PRIVATEIP
# TNUMBER regex is for test PRIVATEIP is a number or not
TNUMBER='^[0-9]+$'
if ! [[ $PRIVATEIP =~ $TNUMBER ]] ; 
then
    echo "✖ Error: It's not a number" 
    echo "So default value selected : 83"
    PRIVATEIP=83       
else
...
--------------------------> 4/4 <-------------------------
     "
echo "> Last step, choose optionnal program you want to install :"
echo "----------------------------------------------------------"
read -p "Press 'enter' to continue or press 'ctl+c' to quit
     "
# ask about PHP install
PHPINST="";
while ! [ "$PHPINST" = "y" -o "$PHPINST" = "n" ]; do
    echo "
--------------------------> PHP <-------------------------
        "
    echo "> Do you want to install PHP7.0 ?"
    echo "> Yes -> y"
    echo "> No  -> n
     "
    read PHPINST
...
then
    # ask about PHP install
    COMPOSERINST="";
    while ! [ "$COMPOSERINST" = "y" -o "$COMPOSERINST" = "n" ]; do
        echo "
------------------------> COMPOSER <----------------------
            "
...
# ask about mysql install
MYSQLINST="";
while ! [ "$MYSQLINST" = "y" -o "$MYSQLINST" = "n" ]; do
    echo "
-------------------------> MYSQL <------------------------
        "
...
--------------------------> 4/4 <-------------------------
        "
    echo "> Ok, so choose MySQL password :"
